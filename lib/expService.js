...
async function grant(user, eventKey, uniqKey) {
    ...
    // VIP (sementara/permanen): SEMUA EXP event x2 — benefit Paket Sultan
    const boost = store.isVip(user) ? 2 : 1
    const amount = ev.xp * boost

    const before = levels.stateFor(cfg.ranks, user.xp)
    const newXp = await store.addXp(user.id, amount)
    if (newXp === null) return { ok: false, reason: 'user_not_found' }
    await store.addXpEvent(user.id, eventKey, amount, ev.label + (boost > 1 ? ' (VIP x2)' : ''), uniq)
